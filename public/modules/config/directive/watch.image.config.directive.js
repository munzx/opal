...
angular.module('expressx').directive('watchImageConfigDirective', ['$modal', '$rootScope', function ($modal, $rootScope) {
	return {
		require: '?ngModel',
		restrict: 'A',
		replace: false,
		...
		link: function (scope, elem, attrs, ngModel) {
			...
			if ( window.FileReader && window.File && window.FileList && window.Blob ){
				...
				$modal.open({
					template: '<h4 class="alert alert-danger">Please use a modern browser to browse expressx!!!</h4>'
				});
			}
		}
	}
